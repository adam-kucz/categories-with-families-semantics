...
module Coer where
  J : ∀{Γ}{A : Ty s Γ}(a₀ : Tm A) →
    Tm (Id-type A a₀ a₀) == Tm (Id-type A a₀ a₀)
  J {Γ = Γ}{A} a₀' = ap Tm {r = _==_}(
    proof {!!}
      === {!!}
        :by: {!!}
    qed)
{-
  J : ∀{Γ}(A : Ty s Γ)(a a' : Tm A) →
    Tm (Id-type A a a' ∙ (𝒑 A ∘ 𝒑 (A ⁺)))
    == Tm (Id' A)
  J {Γ = Γ} A a a' = ap Tm {r = _==_}(
    proof Id-type A a a' ∙ (𝒑 A ∘ 𝒑 (A ⁺))
      === Id' A ∙ (b' ∘ bar a) ∙ (𝒑 A ∘ 𝒑 (A ⁺))
        :by: Id.refl _
      === Id' A ∙ ((b' ∘ bar a) ∘ (𝒑 A ∘ 𝒑 (A ⁺)))
        :by: ∙-comp== (Id' A) _ (𝒑 A ∘ 𝒑 (A ⁺))
      === Id' A ∙ (b' ∘ bar a ∘ 𝒑 A ∘ 𝒑 (A ⁺))
        :by: ap (Id' A ∙_) $ assoc _ (𝒑 A) (𝒑 (A ⁺))
      === Id' A ∙ (b' ∘ (bar a ∘ 𝒑 A) ∘ 𝒑 (A ⁺))
        :by: ap (λ — → Id' A ∙ (— ∘ 𝒑 (A ⁺))) $ sym $ assoc b' (bar a) (𝒑 A)
      === Id' A ∙ (b' ∘ id (Γ ,, A) ∘ 𝒑 (A ⁺))
        :by: {!!}
      === Id' A
        :by: {!!}
    qed)
    where b' = wk-bar a' (just Γ +,, A)
-}
...
{-
  sub-sub : ∀{Γ}{A : Ty s₀ Γ}{B : Ty s₁ (Γ ,, A)}
    (C : Ty s₂ (Γ ,, A ,, B))
    (a : Tm A)
    (b : Tm B)
    → --------------------------
    C ∙ (bar b ∘ bar a) == (C ∙ {!wk-bar (b ⊙ bar a) (just Γ +,, A) ∘ bar a!})
-}
